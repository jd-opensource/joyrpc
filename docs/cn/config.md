...
|provider|cacheKeyGenerator|String|否|json|cache key生成器名称|
...
|consumer|filter|String|否| |过滤器插件名称，多个用英文逗号隔开|
|consumer|registry|ref|**是**| |注册中心配置引用 |
...
|method|cache|Boolean|否| false |provider/consumer|是否开启结果缓存。如果开启需要指定cacheProvider|
|method|compress|String|否| |provider/consumer|压缩算法（启动后是否压缩还取决于数据包大小）|
|method|dstParam|int|否| |consumer|目标参数（机房/分组等）索引，从0开始计数|
|method|cacheProvider|String|否|caffeine|provider/consumer|结果缓存插件名称，默认提供了caffeine、guava和map缓存插件，需要引用相关的类库才能启用|
|method|cacheKeyGenerator|String|否|json|provider/consumer|缓存键生成器名称，系统内置了json和Spring环境下的spel表达式生成器|
...
|method|cacheKeyExpression|String|否| |provider/consumer|缓存键表达式，用于表达式缓存键生成器，如spel|
