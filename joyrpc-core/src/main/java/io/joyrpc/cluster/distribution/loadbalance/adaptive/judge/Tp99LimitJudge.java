...
public class Tp99LimitJudge extends AbstractJudge implements MetricAware {
    ...
    public Rank score(final NodeMetric metric, final AdaptiveConfig config) {
        ...
        if (nodeTp.getRequests() == 0 && metric.isWeak()) {
            ...
            result = config.getTpScore().score(metric.getNodeFunction().apply(nodeTp), RankScore.INT_DESCENDING);
        }
        //先考虑TP，再考虑可用率
        switch (result) {
            case Good:
                return config.getAvailabilityScore().score(nodeTp.getAvailability(), RankScore.DOUBLE_ASCENDING);
            case Fair:
                score = config.getAvailabilityScore().score(nodeTp.getAvailability(), RankScore.DOUBLE_ASCENDING);
