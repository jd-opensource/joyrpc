...
public class PojoUtils {
    ...
    private static Object generalize(Object pojo, Map<Object, Object> history) {
        ...
        if (ClassUtils.isPrimitives(pojo.getClass())) {
            return pojo;
        }
        ...
        for (Method method : pojo.getClass().getMethods()) {
            if (ClassUtils.isReader(method)) {
                try {
                    map.put(ClassUtils.getReaderProperty(method),
        ...
        for (Field field : pojo.getClass().getFields()) {
            if (ClassUtils.isPublicInstanceField(field)) {
                try {
        ...
        try {
            ...
            for (Field field : cls.getFields()) {
                if (fieldName.equals(field.getName())
                        && ClassUtils.isPublicInstanceField(field)) {
                    result = field;
                    break;
                }
            }
    ...
    public static boolean isPojo(Class<?> cls) {
        return !ClassUtils.isPrimitives(cls)
