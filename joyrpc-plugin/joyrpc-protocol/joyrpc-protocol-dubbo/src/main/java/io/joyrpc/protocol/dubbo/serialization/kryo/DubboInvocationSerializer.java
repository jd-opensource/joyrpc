 ...
 * DubboInvocation序列化
 */
public class DubboInvocationSerializer extends AutowiredObjectSerializer {
    ...
    public void write(final Kryo kryo, final Output output, final Object object) {
        try {
            ((DubboInvocation) object).write(new KryoWriter(kryo, output));
        } catch (IOException e) {
    ...
    public Object read(final Kryo kryo, final Input input, final Class type) {
        try {
            return new DubboInvocation().read(new KryoReader(kryo, input));
        } catch (IOException e) {
            throw new KryoException(e.getMessage(), e);
        }
    }
