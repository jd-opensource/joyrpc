    ...
    protected static class ConsulRegistryController extends RegistryController<ConsulRegistry> {
        ...
        protected CompletableFuture<Void> doRegister(final Registion registion) {
            ...
            ServiceOptions opts = new ServiceOptions()
                    .setName(registion.getService())
                    .setId(cr.uuid)
                    .setTags(Collections.singletonList(registion.getString(ALIAS_OPTION)))
                    .setMeta(getMeta(registion.getUrl()))
                    .setCheckOptions(new CheckOptions().setTtl(registry.ttl + "ms").setStatus(CheckStatus.PASSING).setDeregisterAfter("1m"))
                    .setAddress(registion.getHost())
                    .setPort(registion.getPort());
        ...
        protected Map<String, String> getMeta(URL url) {
            ...
            Maps.put(result,KEY_APPAPTH, context.getString(KEY_APPAPTH));
            Maps.put(result,KEY_APPID, context.getString(KEY_APPID));
            Maps.put(result,KEY_APPNAME, context.getString(KEY_APPNAME));
            Maps.put(result,KEY_APPINSID, context.getString(KEY_APPINSID));
            Maps.put(result,JAVA_VERSION_KEY, context.getString(KEY_JAVA_VERSION));
            Maps.put(result,VERSION_KEY, context.getString(PROTOCOL_VERSION_KEY));
            Maps.put(result,Region.REGION, registry.getRegion());
            Maps.put(result,Region.DATA_CENTER, registry.getDataCenter());
            Maps.put(result,BUILD_VERSION_KEY, String.valueOf(Version.BUILD_VERSION));
            if (url.getBoolean(SSL_ENABLE)) {
                result.put(SSL_ENABLE.getName(), "true");
            }
            Maps.put(result,SERIALIZATION_OPTION.getName(), url.getString(SERIALIZATION_OPTION));
            Maps.put(result,WEIGHT_OPTION.getName(), String.valueOf(url.getInteger(WEIGHT_OPTION)));
            Maps.put(result,TIMESTAMP_KEY, String.valueOf(SystemClock.now()));
            Maps.put(result,PROTOCOL_KEY, url.getProtocol());
            Maps.put(result,ALIAS_OPTION.getName(), url.getString(ALIAS_OPTION));
