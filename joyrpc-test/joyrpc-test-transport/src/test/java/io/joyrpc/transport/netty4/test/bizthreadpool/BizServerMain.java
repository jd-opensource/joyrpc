...
public class BizServerMain {
    ...
    public static void main(String[] orgs) throws InterruptedException {
        ...
        server.setBizThreadPool((ThreadPoolExecutor) Executors.newFixedThreadPool(50));
        server.open();

        synchronized (BizServerMain.class) {
            while (true) {
                try {
                    BizServerMain.class.wait();
